...
class BlockRelationship(Base):
    ...
    blocker_profile_id = Column(UUID(as_uuid=True), ForeignKey('user_profile.id'), primary_key=True, default=uuid.uuid4)
    blocked_profile_id = Column(UUID(as_uuid=True), ForeignKey('user_profile.id'), primary_key=True, default=uuid.uuid4)
